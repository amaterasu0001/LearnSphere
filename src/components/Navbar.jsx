...
const Navbar = () => {
  ...
  const handleRestrictedNavigation = (e, route) => {
    ...
    if (!isLoggedIn) {
      ...
      e.preventDefault();
      alert("You must be logged in as a tutor to access this section.");
    } else if (route === "/TutorRequest" && role === "tutor") {
      e.preventDefault();
      alert("You must be logged in as a student to access this section.");
    } else if ((route === "/favourites" || route === "/Profile") && role === "student") {
      e.preventDefault();
      alert("You must be logged in as a tutor to access this section.");
    } else {
      navigate(route);
            ...
            <li className='nav-item mx-2'>
              {isLoggedIn && (
                <Link
                  ...
                  onClick={(e) =>
                    handleRestrictedNavigation(e, `/chat/${localStorage.getItem("studentId") || "general"}`)
                  }
                  style={{ color: "white" }}>
                  Chat
                </Link>
